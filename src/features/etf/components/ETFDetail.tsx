...
const ETFDetail: React.FC<ETFDetailProps> = ({ etf, onBack, isDarkMode }) => {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-6xl mx-auto pb-20">
      ...
      <section
        className={`pb-6 border-b ${isDarkMode ? "border-slate-800" : "border-gray-200"}`}
      >
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <span
            className={`px-3 py-1 rounded-xl text-sm font-mono font-black ${isDarkMode ? "bg-slate-800 text-primary-accent" : "bg-gray-100 text-primary"}`}
          >
            {etf.code}
          </span>
          {etf.sector && (
            <span
              className={`px-3 py-1 rounded-xl text-sm font-bold border ${isDarkMode ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-gray-50 border-gray-200 text-gray-600"}`}
            >
              {etf.sector}
            </span>
          )}
      ...
      <section className="mt-10 grid grid-cols-1 lg:grid-cols-[180px_minmax(0,1fr)] gap-6">
        ...
        <div
          className={`rounded-[24px] border px-8 py-8 ${isDarkMode ? "bg-[#112240] border-slate-700" : "bg-white border-gray-200 shadow-sm"}`}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-1">
            <div>
              ...
              <InfoRow
                label="AUM(CNY 백만)"
                value={formatNumber(etf.aumCnyMillion, 1)}
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="AUM(KRW억원)"
                value={formatNumber(etf.aumKrwBillion, 1)}
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="최소매수단위"
                value={
                  etf.minimumPurchaseUnit
                    ? `${formatNumber(etf.minimumPurchaseUnit, 0)}주`
                    : "-"
                }
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="최소매수금액"
                value={
                  etf.minimumPurchaseAmountKrw
                    ? `${formatNumber(etf.minimumPurchaseAmountKrw, 0)}원`
                    : "-"
                }
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="벤치마크 명칭"
                value={etf.benchmarkNameEn || "-"}
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="대분류 / 소분류"
                value={`${etf.categoryLarge || "-"} / ${etf.categorySmall || "-"}`}
                isDarkMode={isDarkMode}
              />
            </div>
            <div>
              ...
              <InfoRow
                label="배당률"
                value={formatPercent(etf.dividendYield)}
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="일평균 거래대금(YTD)"
                value={formatNumber(etf.avgTradingValueYtdBillion, 1)}
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="NAV"
                value={formatNumber(etf.nav, 4)}
                isDarkMode={isDarkMode}
              />
              <InfoRow
                label="1M"
      ...
      <section
        className={`mt-8 rounded-[24px] border px-8 py-7 ${isDarkMode ? "bg-[#112240] border-slate-700" : "bg-white border-gray-200 shadow-sm"}`}
      >
        <h2
          className={`text-lg font-black ${isDarkMode ? "text-white" : "text-gray-900"}`}
        >
          ETF 개요
        </h2>
        <p
          className={`mt-4 whitespace-pre-line text-[15px] leading-8 ${isDarkMode ? "text-slate-300" : "text-gray-700"}`}
        >
