...
import { Stock, DbIssueRow, StockIssue } from '@/shared/types';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 이슈를 stock_id로 그룹
        const issuesByStock: Record<string, (StockIssue & { id: string })[]> = {};
        if (issuesRes.data) {
          (issuesRes.data as DbIssueRow[]).forEach((issue) => {
            ...
            const mapped = {
              ...
              images: imageUrls.map((img) => ({ url: typeof img === 'string' ? img : img.url, source: '', date: '' })),
            };
        ...
        if (stocksRes.data) {
          const assembled: Stock[] = (stocksRes.data as PartialStockRow[]).map((row) => ({
            id: row.id,
            ticker: row.ticker,
            name: row.name,
            nameKr: row.name_kr,
    ...
    return (
      <div className="flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm font-bold text-slate-200">뉴스를 불러오는 중...</span>
        </div>
      </div>
    );
  }
