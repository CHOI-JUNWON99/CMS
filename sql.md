...
CONSTRAINT etfs_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id)
);
...
category text NOT NULL CHECK (category = ANY (ARRAY['기업'::text, '정책'::text])),
date text NOT NULL,
file_size text,
file_url text,
created_at timestamp with time zone DEFAULT now(),
...
CONSTRAINT resources_stock_id_fkey FOREIGN KEY (stock_id) REFERENCES public.stocks(id),
