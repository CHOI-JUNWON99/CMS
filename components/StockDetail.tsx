...
const StockDetail: React.FC<StockDetailProps> = ({ stock, onBack, isDarkMode }) => {
  ...
  const formatMarketCapDetail = (capStr: string) => {
    ...
    const okPart = parts[1].replace('억원', '').replace('억', '');
    return (
      <span className={`inline-flex items-baseline ${isDarkMode ? 'text-slate-200' : 'text-gray-900'}`}>
        <span className="text-lg xs:text-xl sm:text-2xl lg:text-3xl font-black tracking-tight">{joPart}</span>
        <span className="text-sm xs:text-base sm:text-lg lg:text-xl font-black mx-0.5">조</span>
        <span className="text-lg xs:text-xl sm:text-2xl lg:text-3xl font-black tracking-tight ml-1">{okPart}</span>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-7xl mx-auto pb-20 relative px-4 lg:px-0">
          ...
          <div className={`w-full lg:w-auto flex flex-col gap-3 xs:gap-4 sm:gap-6 px-4 xs:px-6 sm:px-10 py-4 xs:py-6 sm:py-8 rounded-2xl sm:rounded-[2rem] border shadow-xl ${isDarkMode ? 'bg-[#112240] border-slate-700 shadow-black/40' : 'bg-white border-gray-200 shadow-gray-200/40'}`}>
            <div className={`grid grid-cols-2 gap-x-4 xs:gap-x-6 sm:gap-x-8 border-b pb-3 xs:pb-4 sm:pb-6 ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
              ...
              <div className="flex flex-col pl-2 xs:pl-4 sm:pl-4">
                <span className={`text-[8px] xs:text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] xs:tracking-[0.15em] sm:tracking-[0.2em] mb-1 sm:mb-1.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>TOTAL RETURN</span>
            ...
            <div className="grid grid-cols-3 gap-x-2 xs:gap-x-4 sm:gap-x-6">
              <div className={`flex flex-col pr-2 xs:pr-4 sm:pr-6 border-r ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                <span className={`text-[9px] xs:text-[11px] sm:text-[13px] font-black uppercase tracking-[0.05em] xs:tracking-[0.1em] sm:tracking-[0.2em] mb-1 sm:mb-1.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>PER</span>
                <div className={`text-lg xs:text-xl sm:text-2xl lg:text-3xl font-black ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  {stock.per ? stock.per.toFixed(1) : '-'} <span className="text-[10px] xs:text-xs sm:text-sm font-bold ml-0.5 sm:ml-1">배</span>
                </div>
              </div>
              <div className={`flex flex-col px-2 xs:px-4 sm:px-6 border-r ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                <span className={`text-[9px] xs:text-[11px] sm:text-[13px] font-black uppercase tracking-[0.05em] xs:tracking-[0.1em] sm:tracking-[0.2em] mb-1 sm:mb-1.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>PBR</span>
                <div className={`text-lg xs:text-xl sm:text-2xl lg:text-3xl font-black ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  {stock.pbr ? stock.pbr.toFixed(1) : '-'} <span className="text-[10px] xs:text-xs sm:text-sm font-bold ml-0.5 sm:ml-1">배</span>
                </div>
              </div>
              <div className="flex flex-col pl-2 xs:pl-4 sm:pl-6">
                <span className={`text-[9px] xs:text-[11px] sm:text-[13px] font-black uppercase tracking-[0.05em] xs:tracking-[0.1em] sm:tracking-[0.2em] mb-1 sm:mb-1.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>PSR</span>
                <div className={`text-lg xs:text-xl sm:text-2xl lg:text-3xl font-black ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  {stock.psr ? stock.psr.toFixed(1) : '-'} <span className="text-[10px] xs:text-xs sm:text-sm font-bold ml-0.5 sm:ml-1">배</span>
                </div>
              </div>
            </div>
          </div>
                      ...
                      {[...stock.businessSegments].sort((a, b) => b.value - a.value).map((seg, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-2 xs:gap-3 mb-2 xs:mb-4">
                            <div className={`w-8 h-8 xs:w-10 xs:h-10 rounded-lg xs:rounded-xl border flex items-center justify-center shrink-0 ${isDarkMode ? 'border-slate-600 bg-slate-700/50' : 'border-gray-300 bg-white'}`}>
      ...
      <div className="mt-12 xs:mt-20">
        ...
        {timelineData.length > 0 ? (
          <div className="relative max-w-5xl mx-auto pl-8 xs:pl-12">
            {/* Continuous vertical line */}
            <div className={`absolute left-0 top-0 bottom-0 w-[2px] ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`} />
            ...
            {timelineData.map((item, idx) => (
              ...
              >
                <div className={`absolute -left-8 xs:-left-12 top-1 ml-[-7px] xs:ml-[-9px] w-4 h-4 xs:w-5 xs:h-5 rounded-full border-[3px] xs:border-4 transition-all group-hover:scale-125 z-10 ${item.isCMS ? 'bg-primary border-white shadow-md' : 'bg-gray-400 border-white'}`} />
          ...
          <div className={`relative w-full max-w-4xl rounded-2xl xs:rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col ${isDarkMode ? 'bg-[#0a192f] border border-slate-700' : 'bg-white'}`}>
            <div className="p-4 xs:p-10 lg:p-14 overflow-y-auto custom-scrollbar">
              ...
              <div className="mb-6 xs:mb-12">
                <h2 className={`text-xl xs:text-3xl font-black mb-4 xs:mb-6 leading-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}><TextWithCMS text={selectedInsight.title} isDarkMode={isDarkMode} /></h2>
                <div className={`p-4 xs:p-8 rounded-xl xs:rounded-[2rem] text-[13px] xs:text-[17px] leading-relaxed font-bold whitespace-pre-wrap ${isDarkMode ? 'bg-slate-800 text-slate-200' : 'bg-gray-50 text-gray-700'}`}><TextWithCMS text={selectedInsight.content} isDarkMode={isDarkMode} hideBadge={true} /></div>
              </div>
